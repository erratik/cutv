<?php
	...
	if( isset( $_GET[ 'merge_items' ] ) ) {
		...
		foreach ( $items as $item ) {
			...
			$post_id = $ids[ 0 ];
			update_post_meta( $post_id , 'wpvr_video_views' , $item[ 'views' ] );
			$done[ 'count_duplicates' ] ++;
			foreach ( $ids as $id ) {
				if( $id != $post_id ) {
		...
		foreach ( $return[ 'items' ] as $item ) {
			...
			*/
			...
			if( $dups ) {
				...
				$return[ 'html' ] .= '<i class="fa fa-cube" ></i><br/>' . __( 'Merge' , WPVR_LANG );
				...
				<div class = "wpvr_manage_wrapper">
					...
					<div class = "wpvr_manage_bulk">
						<div class = "wpvr_manage_bulk_left pull-left noMargin">
							...
							<?php if( $is_DT && false ) { ?>
								<button url = "<?php echo $wpvr_url; ?>" class = "wpvr_button wpvr_black_button pull-left wpvr_merge_all_duplicates">
									<i class = "wpvr_button_icon fa fa-star"></i>
						...
						<div class = "wpvr_manage_bulk_right  noMargin">
							...
							<div class = "wpvr_manage_bulk_actions">
								...
								<select class = "wpvr_manage_bulk_actions_select pull-left" style = "display:none;">
									...
									<?php if( $is_DT ) { ?>
										<option class = "" value = "merge">
											<?php _e( 'Merge duplicates' , WPVR_LANG ); ?>
										</option>
									<?php } else { ?>
							...
							<?php if( $is_DT ) { ?>
								...
								<div class = "wpvr_manage_box wpvrOrder open">
									...
									<div class = "wpvr_manage_box_content">
										...
										<label>
											<?php _e( 'Find Duplicates By' , WPVR_LANG ); ?>
										</label><br/>
										<select name = "dupsBy">
											<option value = "video_id" selected = "selected"><?php _e( 'Video Id' , WPVR_LANG ); ?></option>
											<option value = "title"><?php _e( 'Video Title' , WPVR_LANG ); ?></option>
										</select>
