<?php
	...
	if( ! function_exists( 'wpvr_make_curl_request' ) ) {
		function wpvr_make_curl_request( $api_url = '' , $api_args = array() , $curl_object = null , $debug = FALSE , $curl_options = array() ) {
	...
	if( ! function_exists( 'wpvr_custom_dashboard_function' ) ) {
		function wpvr_custom_dashboard_function() {
			...
			$newVideoLink  = get_bloginfo( 'url' ) . '/wp-admin/post-new.php?post_type=' . WPVR_VIDEO_TYPE;
			$newSourceLink = get_bloginfo( 'url' ) . '/wp-admin/post-new.php?post_type=' . WPVR_SOURCE_TYPE;
			$dashboardLink = get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=wpvr';
			$deferredLink  = get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=wpvr-deferred';
			$reviewLink    = get_bloginfo( 'url' ) . '/wp-admin/edit.php?post_status=pending&post_type=' . WPVR_VIDEO_TYPE;
			$optionsLink   = get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=wpvr-options';
			$manageLink    = get_bloginfo( 'url' ) . '/wp-admin/admin.php?page=wpvr-manage';
			?>
			...
			<div>
				<div>
					...
					<script>
						var data_videos_by_status = [
							<?php foreach( $video_stats[ 'byStatus' ][ 'items' ] as $label=>$count){ ?>
				...
				<div class = "wpvr_widget_legend">
					...
					<div class = "wpvr_dashboard_center">
						<a href = "<?php echo $newVideoLink; ?>" class = "wpvr_dashboard_button">
							<i class = "wpvr_link_icon fa fa-plus"></i>
							<?php _e( 'Add New Video' , WPVR_LANG ); ?>
						</a>
						<a href = "<?php echo $reviewLink; ?>" class = "wpvr_dashboard_button">
							<i class = "wpvr_link_icon fa fa-flag"></i>
							<?php _e( 'Review Videos' , WPVR_LANG ); ?>
						</a>
						<a href = "<?php echo $deferredLink; ?>" class = "wpvr_dashboard_button">
			...
			<div>
				<div>
					...
					<script>
						var data_sources_by_status = [
							<?php foreach( $sources_stats[ 'byType' ] as $label=>$count){ ?>
							<?php if( $label == 'total' ) continue; ?>
							<?php if( $label == 'groupType' ) $label = 'group_dm'; ?>
							{
								value: parseInt(<?php echo $count; ?>),
								color: '<?php echo $wpvr_colors[ 'sourceTypes' ][ $label ]; ?>',
								label: '<?php echo strtoupper( $wpvr_types_[ $label ][ 'label' ] ); ?>',
							},
							<?php } ?>
						];
						jQuery(document).ready(function ($) {
				...
				<div class = "wpvr_widget_legend">
					...
					<div class = "wpvr_dashboard_center">
						<a href = "<?php echo $newSourceLink; ?>" class = "wpvr_dashboard_button">
							<i class = "wpvr_link_icon fa fa-plus"></i>
			...
			<div class = "wpvr_dashboard_center">
				<a href = "<?php echo $dashboardLink; ?>" class = "wpvr_dashboard_button">
					<i class = " wpvr_link_icon fa fa-dashboard"></i>
					<?php _e( 'View Dashboard' , WPVR_LANG ); ?>
				</a>
				<a href = "<?php echo $manageLink; ?>" class = "wpvr_dashboard_button">
					<i class = "wpvr_link_icon  fa fa-film"></i>
					<?php _e( 'Manage Videos' , WPVR_LANG ); ?>
				</a>
				<a href = "<?php echo $optionsLink; ?>" class = "wpvr_dashboard_button">
	...
	if( ! function_exists( 'wpvr_get_act_data' ) ) {
		function wpvr_get_act_data( $product_slug = 'wpvr' ) {
			...
			//new dBug( $act );
			$wpvr_activation = $wpvr_activations[ $product_slug ];
			if( $wpvr_activation != '' ) {
				$act [ 'act_status' ] = $wpvr_activation[ 'act_status' ];
				$act[ 'act_id' ]      = $wpvr_activation[ 'act_id' ];
				$act[ 'act_email' ]   = $wpvr_activation[ 'act_email' ];
				$act[ 'act_code' ]    = $wpvr_activation[ 'act_code' ];
				$act[ 'act_date' ]    = $wpvr_activation[ 'act_date' ];
				$act[ 'buy_date' ]    = $wpvr_activation[ 'buy_date' ];
				$act[ 'buy_user' ]    = $wpvr_activation[ 'buy_user' ];
				$act[ 'buy_licence' ] = $wpvr_activation[ 'buy_licence' ];
				$act[ 'act_addons' ]  = $wpvr_activation[ 'act_addons' ];
			}
	...
	if( ! function_exists( 'wpvr_add_notice' ) ) {
		function wpvr_add_notice( $notice = array() , $unique = TRUE ) {
	...
	if( ! function_exists( 'wpvr_remove_notice' ) ) {
		function wpvr_remove_notice( $notice_slug ) {
			$notices = get_option( 'wpvr_notices' );
			if( $notices == '' ) $notices = array();
