	...
	function wpvr_video_custom_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'video_data':
				...
				$video_youtube_url = get_post_meta( $post->ID , 'wpvr_video_service_url' , TRUE );
				$video_views = get_post_meta( $post->ID , 'wpvr_video_views' , TRUE );
				$videoData = get_post($post->ID );

				$videoDescription = $videoData->post_content;
				...
				<div class="video-info" data-youtube-url="<?php echo $video_youtube_url; ?>" data-youtube-preview="<?php echo $video_youtube_preview; ?>" data-youtube-postdate="<?php echo $video_youtube_post_date; ?>">
					...
					<?php if( $videoDescription != '' && $videoTitle != '') { ?>
					...
					<a href="#TB_inline?width=600&height=550&inlineId=add-snaptube-modal_<?php echo $post->ID; ?>" class="thickbox">Add to Snaptube</a>

					<div id="add-snaptube-modal_<?php echo $post->ID; ?>" style="display:none;">
					</div>
