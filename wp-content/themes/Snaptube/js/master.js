...
jQuery(document).ready(function($) {
	...
	if ( jQuery('#video_carousel:not(.featured-videos-slider)').length != 0 ) {
	...
	jQuery( function() {
		...
		jQuery('#sorts').on( 'click', 'div:not(.sort_by)', function() {
			...
			if (sortValue == 'views' || sortValue == 'date' ) {
				$container.isotope({
					sortBy: sortValue,
					sortAscending: false
				});
	...
	jQuery( function() {
		...
		if ( jQuery.cookie('vh_category_sorting') == null ) {
			jQuery.cookie('vh_category_sorting', 'date', { path: '/' });
		} else if ( jQuery.cookie('vh_category_sorting') == 'date' || jQuery.cookie('vh_category_sorting') == 'views' ) {
		...
		} else if ( jQuery.cookie('vh_category_sorting') == 'title' ) {
			if ( !container.hasClass('followed') ) {
				jQuery('.video-block-container.open-video').isotope({
					sortBy: sortValue,
					sortAscending: true
		...
		if ( jQuery.cookie('vh_category_sorting') == 'date' ) {
			jQuery('.sort_param.date').removeClass('clicked');
		...
		} else if ( jQuery.cookie('vh_category_sorting') == 'views' ) {
			jQuery('.sort_param.date').removeClass('clicked');
			jQuery('.sort_param.title').removeClass('clicked');
			jQuery('.sort_param.views').removeClass('clicked');
