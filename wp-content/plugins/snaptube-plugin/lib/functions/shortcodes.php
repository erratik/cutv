					...
					if($file_type == 5 && !empty($relFet->embedcode)){
						...
						if( $mobile === true ) {
							...
							if (preg_match("/www\.youtube\.com\/watch\?v=[^&]+/", $reafile, $vresult)) {
								...
								$player_values = '<video id="video" poster="' . $imageFea . '"   src="' . $reavideourl .'" autobuffer controls >' . __( 'Html5 Not support This video Format.', 'vh' ) . '</video>';
							}
						} else {
							## Flash player code
						...
						if($file_type == 5 && !empty($embed_code[$j])){
							...
							if( $mobile === true ) {
								...
								if (preg_match("/www\.youtube\.com\/watch\?v=[^&]+/", $reafile, $vresult)) {
									...
									## Generate video code for html5 player
									$player_values = '<video id="video" poster="' . $imageFea[$j] . '"   src="' . $reavideourl .'" autobuffer controls >' . __( 'Html5 Not support This video Format.', 'vh' ) . '</video>';
								}
							} else {
								## Flash player code
											...
											if ( strpos($files[$j],'soundcloud.com') !== false ) {
												...
												if( strpos($files[$j], 'v=') !== false ) {
													...
													if ( $file_type == 3 ) {
														...
														$div .= '<video width="100%" controls><source src="' . $videourl . '" type="video/mp4">' . __("Your browser does not support the video tag.", "vh") . '</video>';
													} else {
														$videourl   = $image_path . $reafile;
														$image_file = $imageFea[$j];
													}
		...
		function home_player() {                ## FUNCTION FOR HOME PAGE STARTS
			...
			if ( get_option('vh_html5_videos') == 'false' || get_option('vh_html5_videos') == false ) {
				...
				$div             .= '<script> current_video('.$homeplayerData->vid.',"'.$homeplayerData->name.'"); </script>';
					...
					$div .="<video id='video' poster='" . $thumb_image . "'   src='" . $videoUrl . "' autobuffer controls>" . __('Html5 Not support This video Format.', 'vh') . "</video>";
				}
					} else {
						## Flash player code
									...
									if( $file_type == 5 && !empty($embed_code) ) {
										...
										if($mobile === true){
											...
											if (preg_match("/www\.youtube\.com\/watch\?v=[^&]+/", $playList->file, $vresult)) {
												...
												## Generate video code for html5 player
												$player_values = htmlentities('<video id="video" poster="' . $imageFea . '"   src="' . $reavideourl .'" autobuffer controls >' . __( 'Html5 Not support This video Format.', 'vh' ) . '</video>');
											}
										} else {
											## Flash player code
