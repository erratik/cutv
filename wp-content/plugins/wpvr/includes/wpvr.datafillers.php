<?php 
	...
	if( isset($_GET['add_fillers_from_preset']) ){
		...
		foreach( $preset_items as $filler ){
			...
			foreach( $videos as $video ){
				...
				if( is_array($wpvr_fillers) && count( $wpvr_fillers) > 0 ){
					foreach( $wpvr_fillers as $filler ){
						...
						}else $data = get_post_meta( $video_id , $filler['from'] , TRUE );
	...
	if( isset($_GET['show_fillers']) ){
		...
		<?php
		$countFillers = 0;
		foreach( $wpvr_fillers as $k=>$filler){
			$countFillers++;
					...
					<div class="inside">
						<div class="main">
							...
							<select class="wpvr_filler_input" name="filler_preset" id="filler_preset">
								...
								<?php foreach( $wpvr_datafillers_presets as $preset ){ ?>
