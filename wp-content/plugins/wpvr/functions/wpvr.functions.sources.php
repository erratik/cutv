<?php
	...
	if( ! function_exists( 'wpvr_get_source_columns_name' ) ) {
		...
		';
			if( ! isset( $wpvr_vs[ $source->service ] ) ) {
				$actions_line = ' <div class = "wpvr_source_links wpvr_service_disabled">SERVICE NOT ENABLED</div> ';
			} else {
	...
	if( ! function_exists( 'wpvr_get_source_columns_info' ) ) {
		function wpvr_get_source_columns_info( $source ) {
			global $wpvr_vs;
			if( ! isset( $wpvr_vs[ $source->service ] ) ) {
				$echo = ' <div class = "wpvr_source_links wpvr_service_disabled">SERVICE NOT ENABLED</div> ';
			} else {
			...
			$more = apply_filters( 'wpvr_extend_source_column_info' , '' , $source );
	...
	if( ! function_exists( 'wpvr_async_fetch_sources' ) ) {
		function wpvr_async_fetch_sources( $sources , $update_old_videos = FALSE , $debug = FALSE ) {
			...
			foreach ( $sources as $source ) {
				...
				$url = wpvr_capi_build_query( WPVR_ACTIONS_URL , array(
					'wpvr_wpload'         => 1 ,
