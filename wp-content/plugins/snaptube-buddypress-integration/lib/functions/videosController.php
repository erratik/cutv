...
if ( class_exists( 'VideoController' ) != true ) {			
	...
	class VideoControllerEdited extends VideoModel {	
         ...
         */
		public function vh_add_newvideo() {			
			global $wpdb;
			...
			if ( $this->_addnewVideo ) {
				$videoName = $video_slug = filter_input( INPUT_POST, 'video_name' );
        		...
        		if ( $videoLinkurl != '' ) {
					...
					if ( strpos( $act_filepath, 'youtube' ) > 0 ) {
						...
						$youtube_data = $this->vh_hd_getsingleyoutubevideo( $match );
						...
						$duration = $this->vh_convert_time($youtube_time);
					...
					if ( ! empty( $duration ) ) {
						...
						$duration       = $this->vh_converttime( $sec );
					} else {
						$duration = '0:00';
					}
				}
				...
				if ( ! empty( $act_filepath ) ) {
					if ( strpos( $act_filepath, 'youtube' ) > 0 || strpos( $act_filepath, 'youtu.be' ) > 0 ) {
						...
						if ( $youtube_data ) {
							...
							$this->vh_render_error( __( 'Could not retrieve Youtube video information', 'hdflvvideoshare' ) );
					}else if ( strpos( $act_filepath, 'dailymotion' ) > 0 ) {			  ## check video url is dailymotion
						$split     = explode( '/', $act_filepath );
						$split_id  = explode( '_', $split[4] );
                ...
                if ( $this->_videoId ) {	
					...
					$this->vh_admin_redirect( 'admin.php?page=newvideo&videoId=' . $this->_videoId . '&update=1' );
		...
		public function vh_convert_time( $time ) {
			if ( $time != null ) {
				$dt = new DateTime();
				...
				if ( $interval->h > 0 ) {
					$duration = $interval->h.":".$interval->i.":".$interval->s;
				} else {
					$duration = $interval->i.":".$interval->s;
				}
			} else {
				$duration = "0:00";
			}
		...
		public function vh_render_error( $message ) {
			?>
		...
		public function vh_admin_redirect( $url ) {								## admin redirection url function starts
			echo '<script>window.open( "' . $url . '","_top",false )</script>';
		}

		## admin redirection url function ends
		public function vh_video_data() {										## getting video data function starts
		 ...
		 *
		 */
		function vh_hd_getsingleyoutubevideo( $youtube_media ) {
		...
		public function vh_get_message() {										## displaying database message function starts
			...
			if ( isset( $videoApply ) ) {											
				if ( $videoActionup =='videodelete' || $videoActiondown == 'videodelete' ) {		
					if ( is_array( $videocheckId ) ) {
						...
						if ( ! $deleteflag ) {
							$this->vh_admin_redirect( 'admin.php?page=video&del=0' );
						} else {
							$this->vh_admin_redirect( 'admin.php?page=video&del=1' );
						}
					}
...
// $videoOBJ = new VideoControllerEdited();

// $videoOBJ->vh_add_newvideo();
// $videoId = $videoOBJ->_videoId;

// if ( $videoId != NULL ) {
// 	$videoOBJ->vh_get_delete();

// 	$gridVideo        = $videoOBJ->vh_video_data();
// 	$videosearchQuery = $videoOBJ->_videosearchQuery;
// 	$searchBtn        = $videoOBJ->_searchBtn;
// 	$displayMsg       = $videoOBJ->vh_get_message();
